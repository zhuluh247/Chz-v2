  ...
  try {
    ...
    if (numMedia > 0) {
      ...
      if (user && user.step === 'awaiting_payment') {
        ...
        twiml.message("Please complete text steps first. Reply 'Menu' to restart.");
        return res.type('text/xml').send(twiml.toString());
      }
    }
    ...
    if (rider) {
      ...
      if (msg.startsWith('accept ')) {
        ...
        return res.type('text/xml').send(timl.toString());
      }
...
async function handleErrandType(from, type, twiml) {
  ...
  if (needsShopping) {
    twiml.message(`📝 *List items you want to buy.*\n\nFormat: Item Price, Item Price\nExample: Beans 2000, Oil 500`);
  } else {
    twiml.message(`📝 *Describe task or pickup:*`);
  }
}
